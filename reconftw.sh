		...
		if [[ $AXIOM != true ]]; then
			...
			for crit in "${severity_array[@]}"; do
				...
				if [[ -s $NOWAF_LIST ]]; then
					nuclei -H "$HEADER" -l "$NOWAF_LIST" -severity "$crit" -nh -rl "$NUCLEI_RATELIMIT" -silent -retries 2 ${NUCLEI_EXTRA_ARGS} -t ${NUCLEI_TEMPLATES_PATH} -j -o "nuclei_output/${crit}_json.txt" 2>>"$LOGFILE" >/dev/null
				fi
				# WAF hosts at slower rate
				if [[ -s $WAF_LIST ]]; then
					local slow_rl
					slow_rl=$((NUCLEI_RATELIMIT / 3 + 1))
					nuclei -H "$HEADER" -l "$WAF_LIST" -severity "$crit" -nh -rl "$slow_rl" -silent -retries 2 ${NUCLEI_EXTRA_ARGS} -t ${NUCLEI_TEMPLATES_PATH} -j -o "nuclei_output/${crit}_waf_json.txt" 2>>"$LOGFILE" >/dev/null
					...
					if [[ $URL_CHECK_ACTIVE == true ]]; then
						...
						if [[ -s .tmp/katana_targets_normal.txt ]]; then
							...
							if [[ $LINES -gt ${CHUNK_LIMIT:-2000} ]]; then
								if [[ $DEEP == true ]]; then
									process_in_chunks .tmp/katana_targets_normal.txt ${CHUNK_LIMIT:-2000} "katana -silent -list _chunk_ -jc -kf all -H $HEADER -c $KATANA_THREADS -d 3 -fs rdn >> .tmp/katana.txt 2>>\"$LOGFILE\" >/dev/null"
								else
									process_in_chunks .tmp/katana_targets_normal.txt ${CHUNK_LIMIT:-2000} "katana -silent -list _chunk_ -jc -kf all -H $HEADER -c $KATANA_THREADS -d 2 -fs rdn >> .tmp/katana.txt 2>>\"$LOGFILE\" >/dev/null"
								fi
							else
								if [[ $DEEP == true ]]; then
									timeout 4h katana -silent -list .tmp/katana_targets_normal.txt -jc -kf all -H "$HEADER" -c "$KATANA_THREADS" -d 3 -fs rdn >>.tmp/katana.txt 2>>"$LOGFILE" >/dev/null
								else
									timeout 3h katana -silent -list .tmp/katana_targets_normal.txt -jc -kf all -H "$HEADER" -c "$KATANA_THREADS" -d 2 -fs rdn >>.tmp/katana.txt 2>>"$LOGFILE" >/dev/null
								fi
							fi
						fi
